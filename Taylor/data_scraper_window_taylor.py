...
def data_check(journal_name, redo=False, start=0):
    # store information
    columns = ['Author', 'Affiliation', 'Publication Date', 'Journal Title', 'Title', 'URL']
    ...
    for index, row in result_df.iloc[start:].iterrows():
        ...
        if pd.isna(affiliation):
            ...
            try:
                ...
                if soup:
                    author_divs = soup.find_all('div', class_='entryAuthor')
                    if author_divs:
                        # Loop through each author div to extract author name and affiliation
                        for author_div in author_divs:
                            if author_div.find('a', class_='author'):
                                author_name = author_div.find('a', class_='author').text
                                authors_list.append(author_name)
                                # authors_list.append(author_name)
                            if author_div.find('span', class_='overlay'):
                                affiliation = author_div.find('span', class_='overlay').text.split('a ')[-1].split('<a')[0]
